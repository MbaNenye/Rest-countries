  ...
  return (
    ...
    {pageNumbers.map(number => (
        <li className="flex rounded shadow" key={number}>
            <button className=" md:flex justify-between md:m-4 md:p-4 rounded m- p-2 font-bold dark:bg-white dark:text-white" onClick={()=>setCurrentPage(number)}> {number}</button>
        </li>
    ))}
        </ul>
